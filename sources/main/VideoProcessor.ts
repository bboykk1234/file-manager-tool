    ...
    return new Promise<Metadata>(function (resolve, reject) {
      processor
        .ffprobe(function (err, data) {
          ...
          if (videoStream.duration !== undefined && +videoStream.duration >= 0 &&
            videoStream.nb_frames !== undefined && +videoStream.nb_frames >= 0
          ) {
            resolve({
          ...
          if (videoStream.duration === "N/A" || videoStream.nb_frames === "N/A") {
            ...
            if ("NUMBER_OF_FRAMES" in videoStream.tags && typeof videoStream.tags.NUMBER_OF_FRAMES === "string") {
              videoStream.nb_frames = videoStream.tags.NUMBER_OF_FRAMES;
            }

            if ("DURATION" in videoStream.tags && typeof videoStream.tags.DURATION === "string") {
              const hmsParts = videoStream.tags.DURATION.split(":");
              videoStream.duration = ((+hmsParts[0]) * 60 * 60 + (+hmsParts[1]) * 60 + (+hmsParts[2])).toString();
            }
          }

          if (videoStream.nb_frames === undefined || videoStream.duration === undefined) {
            reject("No duration or number of frames in metadata tags");
            return;
          }
